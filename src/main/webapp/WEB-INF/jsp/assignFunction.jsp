...
<script type="text/javascript">
	$(function(){
				...
				$.get("findFunctionByRoleId.do",
					...
					function(data){
						...
						if(data && data.data.length>0){
							$.each(data.data,function(index,obj){
								$("#functionList").treegrid("checkNode",obj.functionid);
							});
						}
					},"json");
			}
		...
		$("#assignBtn").click(function(){
			...
			if(rows.length == 1 && rows[0].functionid == 1){
				$.messager.alert("警告","请至少选择一个子菜单");
				return;
			}
			var ids = new Array();
			$.each(rows,function(index,obj){
				ids.push(obj.functionid); // [1,2,3,4,5]
			});
			ids = ids.join(","); // "1,2,3,4,5"
			//提交修改的菜单选项信息
			$.get(
				"saveRoleFunction.do",
				{"roleid":$("#roleid").val(),"ids":ids},
				function(data){
					alert(data.msg);
					if(data.success){
