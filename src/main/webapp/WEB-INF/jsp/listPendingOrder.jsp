...
<script type="text/javascript">
	$(function(){
		$("#orderList").datagrid({
			...
			columns : [[
				...
				{field:"fromid",title:"订货人ID",sortable:true,width:15},
	...
	function accept(url){
		...
		if(rows.length <= 0 ){
			$.messager.alert("警告","必须选中一条订单接受！");
			return;
		} else if(rows.length > 1){
			$.messager.alert("警告","只能选中一条订单接受！");
			return;
		}else{
	...
	function refuse(url){
		var rows = $("#orderList").datagrid("getChecked");
		if(rows.length <= 0 ){
			$.messager.alert("警告","必须选中一条订单拒绝！");
			return;
		} else if(rows.length > 1){
			$.messager.alert("警告","只能选中一条订单拒绝！");
			return;
		}else{
			var id = rows[0].orderid;
			$.get(
...
<body>
	<div id="toolbar">
		<a href="javascript:void(0);" onclick="return accept('updateOrder.do')" 
			class="easyui-linkbutton" data-options="iconCls:'icon-ok',plain:true">接受订单</a> 
		<a href="javascript:void(0);" onclick="return refuse('updateOrder.do');"
			id="editBtn" class="easyui-linkbutton" data-options="iconCls:'icon-no',plain:true">拒绝订单</a> 
