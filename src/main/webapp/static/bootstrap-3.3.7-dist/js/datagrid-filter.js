(function($){
	...
	var insertRow1 = $.fn.datagrid.methods.insertRow;
	$.extend($.fn.datagrid.methods, {
		...
		insertRow: function(jq, row){
			jq.each(function(){
				...
				if (state.filterSource){
					state.filterSource.total++;
					state.filterSource.rows.splice(row.index,0,row.row);
				}
			});
			var result = insertRow1.call($.fn.datagrid.methods, jq, row);
			return result;
		},
	...
	var extendedOptions = {
		...
		filterMatcher: function(data){
				...
				if (name == 'treegrid'){
					...
					$.map(data.rows, function(row){
						...
						if (isMatch(row, row[opts.idField])){
							rr[row[opts.idField]] = row;
							//getChildRow(data.rows,row.id,rr);
			...
			function getChild(row,rr){
				if(row&&row.length!=0){
					for(var i=0;i<row.length;i++){
						var child=row[i];
						rr[child[opts.idField]]=child;
						getChild(child);
					}
				}
			}
			...
			function getChildRow(rows,id,rr){
				for(var i=0; i<rows.length; i++){
					var row = rows[i];
					if (row._parentId == id){
						rr[row[opts.idField]]=row;
						getChildRow(rows,row.id,rr);
					}
				}
			}
		},
	...
	function init(target, filters){
		...
		opts.loadFilter = function(data, parentId){
			...
			var f=myLoadFilter.call(this, d, parentId);
			return f;
		};
